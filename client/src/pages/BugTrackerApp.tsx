    ...
    return (
        <div className="space-y-6 animate-fade-in">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard title="Total Tickets" value={totalTickets} icon={Bug} color="bg-indigo" />
                <StatCard title="Critical Issues" value={criticalTickets} icon={AlertCircle} color="bg-ember" />
                <StatCard title="Resolved" value={resolvedTickets} icon={CheckCircle2} color="bg-global-green" />
                <StatCard title="Error Logs" value={errorLogs} icon={Terminal} color="bg-wine" />
            </div>
                    ...
                    <div className="space-y-4">
                        {tickets.slice(0, 5).map(ticket => (
                            <div key={ticket.id} className="flex items-center gap-3 pb-3 border-b border-light-border dark:border-dark-border last:border-0 last:pb-0">
                                <div className={`w-2 h-2 rounded-full ${ticket.status === 'resolved' ? 'bg-global-green' : 'bg-indigo'}`} />
                        ...
                        {tickets.length === 0 && <p className="text-gray-400 text-sm">No recent activity</p>}
                    </div>
                </div>
                ...
                <div className="card-modern p-6">
                    ...
                    <div className="space-y-4">
                        <div className="flex items-center justify-between">
                            <span className="text-sm dark:text-gray-300">Error Rate</span>
                            <span className="text-sm font-bold text-red-500">{logs.length > 0 ? ((errorLogs / logs.length) * 100).toFixed(1) : 0}%</span>
                        </div>
                        ...
                        <p className="text-xs text-gray-400 mt-2">Based on {logs.length} captured logs</p>
                    </div>
                </div>
            </div>
        </div>
...
const KanbanView: FC<{
    ...
    const columns: { id: Ticket['status']; label: string; color: string }[] = [
        { id: 'open', label: 'Open', color: 'border-t-indigo' },
        { id: 'in-progress', label: 'In Progress', color: 'border-t-wine' },
        { id: 'resolved', label: 'Resolved', color: 'border-t-global-green' },
            ...
            {columns.map(col => (
                ...
                >
                    <div className="kanban-header">
                        <h4 className="font-bold text-sm text-gray-700 dark:text-gray-300 uppercase tracking-wider">{col.label}</h4>
                    ...
                    {tickets.filter(t => t.status === col.id).map(ticket => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-2">
                                <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded border ${ticket.priority === 'critical' ? 'text-red-500 border-red-200 bg-red-50' :
                                    ticket.priority === 'high' ? 'text-orange-500 border-orange-200 bg-orange-50' :
                                        'text-blue-500 border-blue-200 bg-blue-50'
                                    }`}>{ticket.priority}</span>
                                <MoreHorizontal className="w-4 h-4 text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity" />
                            </div>
                            <h5 className="font-bold text-sm mb-1 dark:text-gray-200 line-clamp-2">{ticket.title}</h5>
                            <div className="flex items-center justify-between mt-3">
                                <span className="text-xs font-mono text-gray-400">#{ticket.id?.slice(0, 4)}</span>
                                {ticket.assignee && (
                                    <div className="w-5 h-5 rounded-full bg-indigo/20 text-indigo flex items-center justify-center text-[10px] font-bold">
                                        {ticket.assignee[0]}
                                    </div>
                                )}
                            </div>
        ...
        <div className="terminal-window animate-scale-in">
            <div className="terminal-header justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="ml-2 text-xs font-mono text-gray-400">console.log --watch</span>
                </div>
                <div className="flex gap-2">
                    {['all', 'error', 'warning', 'info'].map(level => (
                        <button
                            ...
                            className={`px-2 py-0.5 text-[10px] uppercase font-bold rounded ${filterLevel === level ? 'bg-white/20 text-white' : 'text-gray-500 hover:text-gray-300'
                                }`}
                        >
                            {level}
                        </button>
            ...
            <div className="terminal-body scrollbar-thin scrollbar-thumb-gray-700 scrollbar-track-transparent">
                {filtered.map((log) => (
                    <div
                        key={log.id}
                    ...
                    >
                        <span className="text-gray-500 shrink-0 w-20">{new Date(log.timestamp).toLocaleTimeString()}</span>
                        ...
                        <span className="font-mono text-gray-300 truncate group-hover:text-white transition-colors">{log.message}</span>
                    </div>
                ))}
                <div ref={bottomRef} />
                {filtered.length === 0 && (
                    <div className="p-4 text-gray-500 text-center font-mono text-sm">-- No logs found --</div>
                )}
            </div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-light-bg dark:bg-dark-bg transition-colors duration-300 pb-10 relative">
            ...
            <div className="fixed inset-0 pointer-events-none overflow-hidden">
                <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-gradient-to-br from-ember/5 via-wine/5 to-transparent rounded-full blur-3xl -translate-y-1/2 translate-x-1/2" />
                <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-gradient-to-tr from-indigo/5 via-navy/5 to-transparent rounded-full blur-3xl translate-y-1/2 -translate-x-1/2" />
            </div>

            {/* --- Header --- */}
            <header className="sticky top-0 z-30 backdrop-blur-xl border-b bg-white/80 border-gray-200/50 dark:bg-dark-surface/80 dark:border-dark-border/50">
                <div className="px-6 py-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="bg-ember/10 p-2 rounded-xl text-ember">
                            <Bug className="w-6 h-6" />
                        </div>
                        <div>
                            <h1 className="text-xl font-bold dark:text-white tracking-tight">Bug Tracker 2.0</h1>
                            <p className="text-xs text-gray-500 font-medium">System Monitoring & Issue Tracking</p>
                        </div>
                    </div>

                    <div className="flex items-center gap-3">
                        {/* View Switcher */}
                        <div className="flex p-1 rounded-lg border bg-gray-50 border-gray-200 dark:bg-dark-surface dark:border-dark-border">
                            {[
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-2 px-3 py-1.5 rounded-md text-sm font-medium transition-all ${view === item.id
                                        ? 'bg-white dark:bg-dark-surface-2 text-ember shadow-sm'
                        ...
                        <button
                            ...
                            className="btn-modern btn-primary text-sm py-2 px-4 shadow-lg shadow-ember/20"
                        >
                ...
                {/* --- Filters Bar --- */}
                <div className="px-6 py-3 border-t border-gray-200/50 dark:border-dark-border/50 flex gap-4 items-center overflow-x-auto">
                    <div className="relative group max-w-xs w-full">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-4 py-1.5 text-sm rounded-lg border bg-white border-gray-200 dark:bg-dark-surface dark:border-dark-border dark:text-white focus:ring-2 focus:ring-ember/20 outline-none transition-all"
                        />
                    </div>
                    ...
                    {/* Mode Switcher */}
                    <div className="flex p-0.5 rounded-lg border bg-gray-100 border-gray-200 dark:bg-dark-surface dark:border-dark-border">
                        <button
                            ...
                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium transition-all ${monitorMode === 'snapshot'
                                    ? 'bg-white dark:bg-dark-surface-2 text-gray-800 dark:text-white shadow-sm'
                    ...
                    {monitorMode === 'snapshot' && (
                        <div className="flex items-center gap-2 flex-1 min-w-[200px]">
                            ...
                            <input
                                ...
                                className="flex-1 bg-transparent text-sm outline-none px-2 text-gray-600 dark:text-gray-300"
                                placeholder="Fetch logs from URL..."
                            />
                            <button
                                onClick={handleFetchConsole}
                                disabled={fetchingConsole}
                                className="text-xs font-bold text-ember hover:underline disabled:opacity-50"
                            >
                    ...
                    {monitorMode === 'realtime' && (
                        <div className="flex items-center gap-3 flex-1">
                            ...
                            {targetApps.length > 0 && (
                                <div className="flex items-center gap-2">
                                    ...
                                    {targetApps.map(app => (
                                        <span
                                            key={app.socketId}
                                            className="px-2 py-0.5 bg-indigo/10 text-indigo text-xs rounded-full font-mono"
                                            title={app.url}
                                        >
                                            {app.appName}
                                        </span>
                            ...
                            <button
                                ...
                                className="ml-auto text-xs font-medium text-ember hover:underline flex items-center gap-1"
                            >
            ...
            <main className="max-w-[1600px] mx-auto p-6">
                {error && (
                    <div className="mb-6 bg-red-500/10 border border-red-500/20 text-red-500 p-4 rounded-xl flex items-center justify-between">
                ...
                {view === 'kanban' && (
                    <KanbanView
                        ...
                        onSelectTicket={(t) => { setSelectedTicket(t); setShowCreateTicket(true); }} // Simplified edit
                    />
                )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className={`modal-glass w-full max-w-2xl animate-scale-in p-0 overflow-hidden`}>
                        ...
                        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                            {selectedLog && (
                                <div className="p-3 bg-gray-900 text-gray-200 font-mono text-xs rounded-lg mb-4 border border-gray-700">
                            ...
                            <div>
                                <label className="block text-sm font-bold mb-1.5 text-gray-700 dark:text-gray-300">Title</label>
                                <input
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold mb-1.5 text-gray-700 dark:text-gray-300">Priority</label>
                                    <select
                                        className="input-modern"
                                ...
                                <div>
                                    <label className="block text-sm font-bold mb-1.5 text-gray-700 dark:text-gray-300">Assignee</label>
                                    <input
                                        className="input-modern"
                        ...
                        <div className="p-6 border-t border-gray-100 dark:border-white/10 flex gap-3 bg-gray-50/50 dark:bg-white/5">
                            <button
                                ...
                                className="btn-modern btn-primary w-full shadow-lg shadow-ember/20"
                            >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className={`modal-glass w-full max-w-2xl animate-scale-in p-0 overflow-hidden`}>
                        <div className="p-6 border-b border-gray-100 dark:border-white/10 flex justify-between items-center">
                            <div>
                                <h2 className="text-xl font-bold dark:text-white flex items-center gap-2">
                                    <Radio className="w-5 h-5 text-ember" />
                                    Setup Real-time Monitoring
                                </h2>
                                <p className="text-sm text-gray-500 mt-1">Add this script to your target application</p>
                            </div>
                            <button
                        ...
                        <div className="p-6 space-y-4">
                            {/* Target URL Display */}
                            <div className="flex items-center gap-3 p-3 bg-indigo/5 dark:bg-indigo/10 border border-indigo/20 rounded-lg">
                                <Globe className="w-5 h-5 text-indigo" />
                                <div className="flex-1">
                                    <span className="text-xs text-gray-500 dark:text-gray-400">Target Application</span>
                                    <p className="font-mono text-sm font-medium text-indigo">{targetUrl || 'http://localhost:3000'}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <pre className="bg-gray-900 text-gray-100 p-4 rounded-lg text-xs font-mono overflow-x-auto whitespace-pre-wrap">
                                    {getInjectScript()}
                                </pre>
                            </div>

                            <div className="bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/20 rounded-lg p-4">
                                <h4 className="font-bold text-blue-800 dark:text-blue-300 text-sm mb-2">📌 Instructions</h4>
                                <ol className="text-xs text-blue-700 dark:text-blue-400 space-y-1 list-decimal list-inside">
                                    <li>Copy the script above</li>
                                    <li>Paste it into the <code className="bg-blue-100 dark:bg-blue-500/20 px-1 rounded">&lt;head&gt;</code> or before <code className="bg-blue-100 dark:bg-blue-500/20 px-1 rounded">&lt;/body&gt;</code> of your target app</li>
