...
const menuItemHover = 'hover:bg-indigo/8 dark:hover:bg-white/5';
const menuItemActive = 'bg-indigo/10 dark:bg-indigo/15 text-indigo dark:text-indigo font-semibold';
...
const ItemContent: FC<ItemContentProps> = ({ item, isCollapsed }) => (
    <div className={`flex items-center ${isCollapsed ? 'gap-0' : 'gap-3'}`}>
        ...
        {item.badge && !isCollapsed && (
            <span className="text-xs bg-ember text-white px-2 py-0.5 rounded-full">{item.badge}</span>
        )}
    </div>
);
            ...
            <li key={item.label}>
                {item.to && item.to !== '#' ? (
                    <NavLink
                        ...
                        onClick={onNavigate}
                        className={({ isActive }) =>
                            `${menuItem} ${isCollapsed ? 'justify-center' : ''} ${
                                isActive ? menuItemActive : `${menuItemHover} text-light-text-secondary dark:text-dark-text-secondary`
                            }`
                        }
                        title={isCollapsed ? item.label : undefined}
                    >
                        <ItemContent item={item} isCollapsed={isCollapsed} />
                    </NavLink>
                ) : item.href ? (
                    <a
                        href={item.href}
    ...
    const Logo: FC<{ showClose?: boolean }> = ({ showClose }) => (
        <div className="flex items-center justify-between px-1 py-1 mb-2">
            <div className="flex items-center gap-2 px-2 py-2">
                <i className="ri-code-s-slash-line text-xl text-indigo" />
                {(!isCollapsed || showClose) && (
                    <h1 className="text-lg font-bold text-light-text dark:text-dark-text">
                        Apex<span className="text-indigo">Ops</span>
                    </h1>
                )}
            </div>
            {showClose ? (
            ...
            <div className={`mx-2 p-2.5 rounded-xl ${collapsed ? '' : 'bg-light-surface-2/60 dark:bg-dark-surface-2/60 border border-light-border/40 dark:border-dark-border/40'}`}>
                <div className={`${collapsed ? 'flex justify-center' : 'flex items-center gap-3'}`}>
                    <img
                        ...
                        alt="profile"
                        loading="lazy"
                        className={`${collapsed ? 'w-10 h-10' : 'w-9 h-9'} object-cover rounded-full ring-2 ring-indigo/20`}
                    />
                    {!collapsed && (
            ...
            {onLogout && !collapsed && (
                <div className="mx-2 mt-2">
                    <button
                        ...
                        className={`w-full ${menuItem} text-ember/80 hover:bg-ember/8 hover:text-ember`}
                    >
    ...
    return (
        <>
            ...
            {/* ════════════════════════════════════════════════════
               ...
               ════════════════════════════════════════════════════ */}
            <div
                className={`hidden lg:flex lg:flex-col h-screen sticky top-0 bg-white dark:bg-dark-bg px-4 py-4 border-r border-light-border/60 dark:border-dark-border/60 shadow-sm transition-[width] duration-300 ease-out overflow-hidden ${
