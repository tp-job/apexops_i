...
const PatientDemographicsChart: FC = () => {
    ...
    const CustomTooltip = ({ active, payload }: RechartsTooltipProps<{ name: string; value: number; label: string }>) => {
        if (active && payload && payload.length) {
            ...
            if (!data) return null;
            return (
                <div className="px-3 py-2 rounded-xl shadow-lg border bg-white border-light-border dark:bg-dark-surface dark:border-dark-border">
            ...
            <div className="p-6">
                ...
                <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {patientDemographics.map((item) => (
                        <div 
                            key={item.name} 
                            className="flex items-center gap-2 p-2 rounded-xl bg-light-surface-2 dark:bg-dark-surface-2"
                        >
                            <div 
